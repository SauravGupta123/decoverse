...
const calculateDecoverseRating = (user) => {
  let maxDecoverseRating = 0;
  ...
  ["codeforces", "codechef", "leetcode"].forEach((platform) => {
    ...
    if (platformData && platformData.rating) {
      ...
      if (platformRating > maxDecoverseRating) {
        maxDecoverseRating = platformRating;
      }
    }
  });

  return maxDecoverseRating;
};
...
const handleUserDataUpdate = async (user) => {
  ...
  user.decoverse_rating = calculateDecoverseRating(user);
  // console.log("new:", user.decoverse_rating);
