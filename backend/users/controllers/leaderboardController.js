...
const getLeaderboard = async (req, res) => {
  try {
    ...
    if (req.query.platform) {
      ...
      users = await User.find({ decoverse_rating: { $gt: 0 } });
      totalUsers = users.length;
      users.sort((a, b) => b.decoverse_rating - a.decoverse_rating);
    }
    ...
    const top3 = users.slice(0, 3).map((user) => {
      ...
      return {
        ...
        name: user.name,
        ...userRatings,
        decoverse_rating: user.decoverse_rating,
        platform_rating: platformRating,
      };
    });

    const allSortedUsers = users;
    // For leaderboard table: if ≤3 users, show all users; if >3 users, show only users after top3
    const usersForPagination = totalUsers > 3 ? users.slice(3) : users;
    users = usersForPagination.slice((page - 1) * pageSize, page * pageSize);
      ...
      const userRatings = {
        ...
        codeforces: user
          ? user.codeforces
            ...
            : null
          : null,
        decoverse_rating: user ? user.decoverse_rating : null,
    ...
    const total_pages = totalUsers > 3 ? Math.ceil((totalUsers - 3) / pageSize) : Math.ceil(totalUsers / pageSize);
    ...
    if (req.query.platform) {
      ...
      leaderboard.sort((a, b) => b.decoverse_rating - a.decoverse_rating);
    }
