...
export default function Leaderboard() {
  ...
  const [selectedRating, setSelectedRating] = useState("decoverse");
  const platforms = ["leetcode", "codechef", "codeforces"];
  const platformsIcon = [leetcode, codechef, codeforces];
  const ratings = ["decoverse", "codechef", "leetcode", "codeforces"];
  ...
  const CustomTooltip = () => {
    return (
      <HtmlTooltip
        title={
          <React.Fragment>
            <Typography color="inherit">Decoverse Rating</Typography>
            <p className="italic">
            ...
            <table className="border border-white rounded-xl">
              <thead>
                <tr className="border border-white">
                  ...
                  <th className="border border-white ">Decoverse Rating</th>
                </tr>
              </thead>
              <tbody>
                <tr className="border border-white">
  ...
  const getRank = (index) => {
    // If we have ≤3 total users, show ranks starting from #1, otherwise start from #4
    return top3.length >= 3 ? index + 4 + (currentPage - 1) * 5 : index + 1;
  };
    ...
    <>
      ...
      <div className="text-white text-center flex flex-col items-center justify-center">
        <h1 className="max-sm:text-[20px] max-sm:leading-6 mt-5 leading-[60px]">
          ...
          <br />
          All Your{" "}
          <span className="bg-decoverse-bg py-[1px] sm:py-1 rounded-md">
            &nbsp;Coding Battles&nbsp;
          </span>
        </h1>
      </div>
      {/* Only show top3 podium if we have more than 3 users total */}
              ...
              <tbody>
                    ...
                    .map((row, index) => (
                      ...
                      >
                        ...
                        <td className="max-sm:hidden">
                          {Math.floor(row.decoverse_rating)}
                        </td>
                        <td className="sm:hidden">
                          {selectedRating === "decoverse"
                            ? Math.floor(row.decoverse_rating)
                ...
                {currentUserData && userDetails && (
                  ...
                  >
                    ...
                    <td>
                      {Math.floor(currentUserData.ratings.decoverse_rating)}
                    </td> */}
                    <td className="max-sm:hidden">
